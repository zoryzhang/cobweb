...
from cobweb.preprocessor import Preprocessor
from cobweb.preprocessor import rename_relation
from cobweb.preprocessor import get_attribute_components
...
def flat_match(target, base, initial_mapping=None):
    ...
    :param target: An instance or concept.av_count object to be mapped to the
        base concept.
    :type target: :ref:`Instance<instance-rep>` or av_count obj from concept
    ...
    cnames = frozenset(get_component_names(base.av_count))
...
def mapping_cost(mapping, target, base):
    ...
    :type target: an instance or concept.av_count
    :param base: the base
    :type base: a concept
    """
    ...
    temp_base = CobwebNode()
    temp_base.update_counts_from_node(base)
    temp_base.tree = base.tree

    # check if it is an av_count table, then create concept to deal with it.
    if isinstance(next(iter(renamed_target.values())), dict):
        temp_target = CobwebNode()
        temp_target.av_count = renamed_target
...
class StructureMapper(Preprocessor):
    ...
    def transform(self, target, initial_mapping=None):
        """
        Transforms a provided target (either an instance or an av_count table
        from a CobwebNode).

        :param target: An instance or av_count table to rename to bring into
            alignment with the provided base.
        :type target: instance or av_count table (from CobwebNode).
        :return: The renamed instance or av_count table
        :rtype: instance or av_count table
        """
    ...
    def undo_transform(self, target):
        ...
        :param target: A previously renamed instance or av_count table to
            reverse the structure mapping on.
        :type target: previously structure mapped instance or av_count table
            (from CobwebNode).
        :return: An instance or concept av_count table with original object
            names
        :rtype: dict
        """
